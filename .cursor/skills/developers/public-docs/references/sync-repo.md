...
| `explore-*/**` | Catalogued app source (un-ignored via `!explore-*/**`; `.env`, `node_modules`, `dist` still denied) |
...
**Not** published by default: project `.cursor/skills/`, secrets (`.env`, `.env.local`),
`node_modules`, build outputs. Application source under `explore-*` is tracked when
`.gitignore` un-ignores `!explore-*/**`; never stage secrets.
...
git add explore-ai/ explore-iam/ explore-chat/ explore-commerce/ explore-lowcode/
git status --short
```
