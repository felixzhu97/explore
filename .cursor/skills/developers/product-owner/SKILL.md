...
## Epic ownership (required)

When creating a Jira **Story / Task / Bug / Feature** (not when creating the Epic itself):

1. **Search** for an existing Epic that owns this outcome (see [mcp](references/mcp.md)).
2. **If a matching Epic exists** — create the child and **link it under that Epic**.
3. **If the work is a new capability/theme with no Epic** — **create the Epic first** (`issueTypeName`: `长篇故事`), business-facing summary; then create the Story/Task and link it.
4. **Never** leave a delivery Story/Task without an Epic when creating via this skill.
5. Epic naming follows **Business language** (outcome theme) — not tool/class names.
...
| Jira MCP + Epic + project config    | [mcp](references/mcp.md)                                 |
...
- [ ] Epic exists or was created; child linked to that Epic ([mcp](references/mcp.md))
