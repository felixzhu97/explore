...
| Published Language       | 发布语言 | Cross-context contract owned by this BC for consumers (e.g. explore-chat BFF) | Integration     | `@explore/contracts-ai` OpenAPI (`/api/text`, `/api/sessions`) | Do not share domain types across BCs; consumers adapt via ACL |
